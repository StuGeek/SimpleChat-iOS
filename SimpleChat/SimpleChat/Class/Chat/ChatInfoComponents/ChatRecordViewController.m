...
@interface ChatRecordViewController ()<EMSearchBarDelegate, EMAvatarNameCellDelegate>
...
{
    ChatViewController *chatController = [[ChatViewController alloc]initWithConversationId:self.conversation.conversationId conversationType:self.conversation.type];
...
- (NSArray *)_formatMessages:(NSArray<EMMessage *> *)aMessages
{
    ...
    for (int i = 0; i < [aMessages count]; i++) {
        ...
        if (self.msgTimelTag < 0 || interval > 60 || interval < -60) {
            timeStr = [DateHelper formattedTimeFromTimeInterval:msg.timestamp];

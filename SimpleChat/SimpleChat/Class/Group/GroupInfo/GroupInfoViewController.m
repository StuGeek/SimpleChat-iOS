...
#import "GroupOwnerViewController.h"
#import "GroupMembersViewController.h"
#import "GroupAdminsViewController.h"
#import "GroupMutesViewController.h"
#import "GroupBlacklistViewController.h"
#import "GroupSharedFilesViewController.h"
...
@interface GroupInfoViewController ()<EMGroupManagerDelegate>
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 0) {
        if (row == 1) {
            //群成员
            GroupAllMembersViewController *controller = [[GroupAllMembersViewController alloc]initWithGroup:self.group];
        ...
        if (row == 0) {
            ...
            //群共享文件
            GroupSharedFilesViewController *controller = [[GroupSharedFilesViewController alloc] initWithGroup:self.group];
            [self.navigationController pushViewController:controller animated:YES];
        } else if (row == 2) {
            ...
            //群管理
            GroupManageViewController *controller = [[GroupManageViewController alloc]initWithGroup:self.groupId];
            [self.navigationController pushViewController:controller animated:YES];
        }
    } else if (section == 2) {
        if (row == 0) {
            ...
            ChatRecordViewController *chatRrcordController = [[ChatRecordViewController alloc]initWithCoversationModel:conversation];
...
- (void)_updateGroupOnwerAction
{
    ...
    GroupOwnerViewController *controller = [[GroupOwnerViewController alloc] initWithGroup:self.group];
    __weak typeof(self) weakself = self;
...
- (void)addMemberAction
{
    ...
    [EMClient.sharedClient.groupManager getGroupSpecificationFromServerWithId:self.groupId completion:^(EMGroup *aGroup, EMError *aError) {
        if (!aError) {
            ...
            InviteGroupMemberViewController *controller = [[InviteGroupMemberViewController alloc] initWithBlocks:occupants];
